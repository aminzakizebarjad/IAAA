...
# class ClassificationDataset(torchvision.datasets.ImageFolder):  #  TODO: changed
class ClassificationDataset(torchvision.datasets.DatasetFolder):  # changed to DatasetFolder to prevent checking image
    # suffix, because DICOM is not an image suffix
    """
    ...
    def __init__(self, root, augment, imgsz, cache=False):
        # super().__init__(root=root)  #  TODO: changed
        super().__init__(root=root, loader=cv2.imread, extensions=(".dcm",))  # the getitem is overloaded at the buttom
        # so the loader is not important and is to prevent errors
    ...
    def __getitem__(self, i):
        ...
        if self.cache_ram and im is None:
            ...
            # im = cv2.imread(f)  # BGR TODO: changed

            im = pydicom.dcmread(f).pixel_array
        if self.album_transforms:
            # sample = self.album_transforms(image=cv2.cvtColor(im, cv2.COLOR_BGR2RGB))['image']  # TODO: changed
            sample = self.album_transforms(image=cv2.cvtColor(im, cv2.COLOR_GRAY2RGB))['image']
        else:
            # sample = self.torch_transforms(im)  # TODO: changed
            sample = self.torch_transforms(image=cv2.cvtColor(im, cv2.COLOR_GRAY2RGB))['image']
