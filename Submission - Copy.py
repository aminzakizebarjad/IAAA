...
class ClassificationPredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, output=None):
        """
    ...
    def stream_inference(self, source=None, model=None):
        ...
        for batch in self.dataset:
            ...
            lb_indx = preds.argmax().item()
            if lb_indx == 0:
                lb_indx = 1
            else:
                lb_indx = 0

            dt1 = pandas.DataFrame({'path': Path(path).name, 'is_patient': [lb_indx]})
...
if __name__ == "__main__":
    ...
    predictor = ClassificationPredictor(overrides=args, output=output)
